...
namespace NativeAnalyzer
{
    ...
    {
    public async Task<string> RunAnalysisAsync(string? baseFolder = null, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var tcpResults = new List<TcpingResult>();

                foreach (var t in targets)
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    var r = await Program.StartParallelTests(t, outputFolder, 60);
                    if (r != null) tcpResults.Add(r);
                    progress?.Report($"Completed tests for {t}");
                }
